...
var _time   : float = 0.0
var _hue    : float = 0.0
var _glow   : float = 0.0   # extra glow on hit

# Public rect for collision
var rect : Rect2 :
...
func _process(delta: float) -> void:
	...
	position.x = clamp(mx, HALF_W + 10, 790 - HALF_W)
	queue_redraw()
...
func _draw() -> void:
	...
	# Body gradient (simulate with 3 rects)
	...
	var gem := PackedVector2Array([Vector2(0,-5), Vector2(6,0), Vector2(0,5), Vector2(-6,0)])
